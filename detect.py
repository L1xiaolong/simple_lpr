import torch
import numpy as np
from model.utils import cfg_mnet, py_cpu_nms, decode, decode_landm, PriorBox
import cv2.cv2 as cv2
from PIL import Image, ImageDraw, ImageFont
from model.net import Retina
from model.rec import plate_recogition
...
def puttext_chinese(img, text, point, color):
    ...
    font = ImageFont.truetype("./font/simhei.ttf", fontsize, encoding="utf-8")
...
class license_plate_ocr:
    def __init__(self, confidence_threshold=0.02, top_k=1000, nms_threshold=0.4, keep_top_k=500, vis_thres=0.6):
        ...
        self.net = load_model(self.net, 'pre_params/mnet_plate.pth', False)
        self.net.eval()
...
def detect(image):
    ...
    model = license_plate_ocr(confidence_threshold=confidence_threshold, top_k=top_k,
                              nms_threshold=nms_threshold, keep_top_k=keep_top_k, vis_thres=vis_thres)
    result_image, img_box, result = model.det_rec(image)
    return result_image, img_box, result
